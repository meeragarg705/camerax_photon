...
public class Equalization extends Node {
    ...
    public void Run() {
        ...
        if (basePipeline.mSettings.DebugData)
            GenerateCurveBitm(histParser.outputArr[1], histParser.outputArr[2], histParser.outputArr[3]);
        ...
        for (int i = 0; i < hist.length; i++) {
            ...
            if (shadowW != 0.f) {
                if (shadowW > 0.f)
                    ...
                    hist[i] = (float) mix(hist[i], (hist[i]) * (hist[i]), -(shadowW) * shadowsSensitivity);
            }
        ...
        desaturate -= 1.0f;
        if (basePipeline.mSettings.DebugData)
            GenerateCurveBitmWB(hist, BLPredictShift, new float[]{WL, WL, WL});
